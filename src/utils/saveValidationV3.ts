...
export const isSaveDataV3Shape = (saveData: SaveData | null | undefined): boolean => {
  ...
  return (
    ...
    && isPlainObject(anySave.轮回者)
    && isPlainObject(anySave.主神空间)
    && isPlainObject(anySave.团队)
...
export function validateSaveDataV3(saveData: SaveData): SaveValidationResult {
  ...
  if (!isSaveDataV3Shape(saveData)) {
    return {
      isValid: false,
      errors: ['不是 V3 存档结构（缺少 元数据/轮回者/主神空间/团队/副本记录/当前副本/社交/世界/系统）'],
      warnings: [],
    };
  }
  ...
  if (!['hub', 'mission', 'settlement'].includes(String(anySave.元数据?.当前阶段 ?? ''))) {
    errors.push('元数据.当前阶段 必须是 hub|mission|settlement');
  }
  errors.push(...validateGameTime(anySave.元数据?.时间));

  if (!isPlainObject(anySave.轮回者?.身份)) errors.push('轮回者.身份 必填且必须是对象');
  if (!isPlainObject(anySave.轮回者?.属性)) errors.push('轮回者.属性 必填且必须是对象');
  if (!isPlainObject(anySave.轮回者?.位置)) errors.push('轮回者.位置 必填且必须是对象');
  if (!Array.isArray(anySave.轮回者?.效果)) errors.push('轮回者.效果 必填且必须是数组');
  if (!isPlainObject(anySave.轮回者?.背包)) errors.push('轮回者.背包 必填且必须是对象');
  if (!isPlainObject(anySave.轮回者?.装备)) errors.push('轮回者.装备 必填且必须是对象');

  const items = anySave.轮回者?.背包?.物品;
  if (!isPlainObject(items)) errors.push('轮回者.背包.物品 必填且必须是对象');

  const equipment = anySave.轮回者?.装备;
  if (isPlainObject(equipment) && isPlainObject(items)) {
    for (let i = 1; i <= 6; i++) {
      ...
      if (v == null) continue;
      if (typeof v !== 'string') {
        errors.push(`轮回者.装备.${key} 必须是 string|null`);
        continue;
      }
      if (!(v in (items as any))) warnings.push(`轮回者.装备.${key} 引用了不存在的物品ID：${v}`);
    }
  }

  if (!isPlainObject(anySave.主神空间)) errors.push('主神空间 必填且必须是对象');
  if (!isPlainObject(anySave.团队)) errors.push('团队 必填且必须是对象');
  if (!Array.isArray(anySave.副本记录)) errors.push('副本记录 必填且必须是数组');
  ...
  // 兼容性提示：角色镜像建议保留，便于旧模块读写
  if (!isPlainObject(anySave.角色)) {
    warnings.push('建议保留角色镜像字段（角色.*），以兼容旧模块');
  }

  // 明确提示废弃结构
  if (isPlainObject(anySave.系统?.扩展) && '无限流' in anySave.系统.扩展) {
    warnings.push('检测到系统.扩展.无限流（已废弃），建议迁移到顶层字段');
  }
