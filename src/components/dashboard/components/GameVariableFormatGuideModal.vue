...
年龄: number (自动计算)
出生日期: {年, 月, 日, 小时?, 分钟?} (只读)
基础六维: {STR, PER, INT, LUK, CHA, WIL} (只读，1-10；兼容先天六司映射)
后天加点: {STR, PER, INT, LUK, CHA, WIL} (可修改，用add；兼容后天六司)</pre>

                <h5>1.2 属性（可修改）</h5>
                <pre class="code-block">评级: {名称, 阶段, 当前进度, 下一级所需, 突破描述}
  → 用set更新整个对象
  → 晋升时必须更新突破描述
                  ...
                  <pre class="code-block">// 添加增益状态
{
  ...
  "value": {
    ...
    "状态描述": "EP恢复速度提升50%"
            ...
            <section id="dao" class="format-section">
              <h4 class="section-title">2. 能力树（兼容大道字段）</h4>
              <div class="section-content">
  ...
  - 总经验: number</pre>
                ...
                <div class="example-box">
                  <strong>示例：</strong>
                  <pre class="code-block">// 解锁并初始化能力分支
{
  "action": "set",
  "key": "大道.大道列表.剑道",
  "value": {
...
// 增加经验
{
  ...
  - 神点: number（推荐主货币，路径：背包.货币.神点.数量）
  - 兼容货币: 灵石 {下品, 中品, 上品, 极品}
  - 物品: {物品ID: Item对象}

物品类型:
  - 装备: {物品ID, 名称, 类型:"装备", 品质, 数量, 描述, 装备增幅, 特殊效果}
  - 能力模块: {物品ID, 名称, 类型:"功法"|"能力芯片"|"能力模块", 品质, 数量, 描述, 功法效果/能力效果, 功法技能[], 修炼进度, 已解锁技能[]}
...
品质格式: {quality:"凡"|"黄"|"玄"|"地"|"天"|"仙"|"神", grade:0-10}</pre>
                ...
                <div class="warning-box">
                  <strong>⚠️ 重要：</strong>
                  <ul>
                    <li>模块熟练度和已解锁技能存储在<strong>角色.背包.物品.{模块ID}</strong>中</li>
                    <li>❌ 错误：修改"角色.修炼.修炼功法"字段（这是引用槽位，不是进度来源）</li>
                    <li>✅ 正确：修改"角色.背包.物品.{模块ID}.修炼进度"</li>
                  </ul>
                </div>

                <div class="example-box">
                  <strong>示例：</strong>
                  <pre class="code-block">// 增加能力模块熟练度
{
  "action": "add",
...
// 解锁新技能
{
  ...
  - 名字: string
  - 性别: "男"|"女"|"其他"
  - 评级: {名称, 阶段} (⚠️ NPC评级是简化结构，严禁添加"当前进度"等玩家专属字段)
  - 与玩家关系: string
  ...
  - 当前内心想法: string (用set实时更新)</pre>
                ...
                <div class="warning-box">
                  <strong>⚠️ NPC评级格式：</strong>
                  <ul>
                    <li>✅ 正确：{"名称": "B", "阶段": "三星"}</li>
                    <li>❌ 错误：{"名称": "B", "阶段": "三星", "当前进度": 0}</li>
                  </ul>
                </div>
...
// 添加记忆
{
  "action": "push",
  "key": "关系.张三.记忆",
  "value": "【1000年3月15日】与玩家协同完成任务，配合顺畅"
}
