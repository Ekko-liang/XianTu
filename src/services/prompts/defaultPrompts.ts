...
import {
  ...
  STATUS_EFFECT_RULES,
  DAILY_EVENT_GUIDANCE_RULES,
  NPC_VISIT_AND_SOCIAL_RULES,
...
## 📝 正文要求（必须遵守）
1. **长度**：统一输出1500-2000字，保证叙事完整、细节充足
2. **章节节奏**：一次互动要有标准网文章节的推进量，不强求单章闭环，但本章必须有明确进展或新的悬念，不能把整段篇幅只耗在到场、寒暄和试探前摇
3. **玩家控制粒度**：玩家输入默认是方向，不是逐动作脚本；在不触及重大决定的前提下，可自动补完低价值细节并推进到下一个关键节点
4. **判定系统**：战斗/修炼/炼丹/探索/社交等场景**必须使用判定**
5. **判定格式（必须严格遵守）**：〔类型:结果,判定值:数字,难度:数字,基础:数字,幸运:+/-数字,环境:+/-数字,状态:+/-数字〕
   - 类型 ∈ 战斗/修炼/炼丹/探索/社交/逃跑/感知
   - 结果 ∈ 大失败/失败/成功/大成功/完美
6. **叙事风格**：多描写少总结，结尾留钩子，承接上文情节
7. **格式标记**：合理使用【】环境、""对话、〔〕判定
8. **画面感配方（最低标准）**：至少2个可见动作细节+1轮对话；【环境】仅在场景变化/信息必要时写1-2句（动作细节必须融入叙事，禁止写成“动作细节一/二”等编号）
...
□ 大境界突破 → set \`角色.属性.境界.名称\`（字符串:凡人→练气→筑基→金丹→元婴→化神→炼虚→合体→渡劫）+ 重置阶段为"初期" + 更新属性上限
...
## 🔴 输出格式（必须严格遵守）
{"text":"1500-2000字开局叙事，第三人称，修仙正剧风"}
...
- 开局也要有章节推进量，但不强求单章闭环；至少落下一个明确事件、变化或悬念
- 玩家后续只控制大方向，不控制每个细节，因此正文允许压缩赶路、寒暄、日常准备等低价值过程
- 严禁游戏术语和数据罗列
- 合理使用【】环境描写、""对话增强表现力
- 画面感配方（最低标准）：至少1段简短【环境】(1-2句)+2-3个可见动作细节+1-2轮对话（动作细节必须融入叙事，禁止写成“动作细节一/二”等编号）
...
- 事件规模必须匹配玩家当前境界的可触达范围：低境界优先生成局部危机/局部机缘/局部人物风波；若世界存在更高阶大事，只能以余波、传闻、封锁、逃难潮、外围征召、资源异动等形式落到玩家身边
- 禁止把远超玩家当前能力上限的主力战场直接压到玩家面前，禁止生成"当前无论怎么选都只能死"的贴脸危机
- 若事件根源远强于玩家，玩家本回合可处理的目标必须降级为：求生、报讯、护送、侦查、藏匿、捡漏、救人、求援其中之一，不能要求玩家独自解决灾难源头
- 必须给出至少一个明确破局点：退路、避难处、可求援对象、可调查线索、可利用环境、可争取资源、可延后处理窗口，至少一项
...
输出JSON（不要代码块/解释/额外文本）：
{
  ...
  "prompt_addition": "一段可直接注入主叙事的事件快照（强调刚刚发生，且必须写出玩家此刻可感知的破局点/退路/线索之一）"
}`,
      category: 'generation',
